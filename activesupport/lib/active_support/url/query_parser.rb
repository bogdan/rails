  ...
  class URL
    class QueryParser
      def initialize(
        ...
        @separator = separator || ActiveSupport::URL::QueryToken::SEPARATOR
      end
      ...
      def parse_token(params, name, value, depth)
        ...
        if depth == 0
          # At depth 0: find the first [ after position 0,
          # treating everything before it as the key.
          # This preserves leading brackets (e.g. "[foo]" stays "[foo]", not "foo").
